// Core Calendar Sync Library for Vercel
// Handles OAuth2, calendar fetching, and Trello integration

import { kv } from '@vercel/kv';

export class CalendarTrelloSync {
  constructor() {
    this.config = {
      OAUTH2: {
  ...
  // Load tokens from Vercel KV storage
  async loadTokens() {
    ...
    try {
      ...
      console.warn('🔑 No tokens found in KV storage.');
      return null;
    } catch (error) {
  ...
  // Save tokens to Vercel KV storage
  ...
  async ensureAuthenticated() {
    console.log('🛡️ Ensuring authentication...');
    const existingTokens = await this.loadTokens();
    
    if (!existingTokens) {
      console.error('❌ No authentication tokens found.');
      throw new Error('No authentication tokens found. Please complete initial setup.');
    }
    
    this.accessToken = existingTokens.access_token;
    this.refreshToken = existingTokens.refresh_token;
    ...
    try {
      ...
      try {
        ...
        console.error('🔥 Failed to refresh access token during ensureAuthenticated:', refreshError);
  ...
  async getCalendarEvents(timeMin, timeMax) {
    console.log(`🗓️ Getting calendar events from ${timeMin} to ${timeMax}...`);
  ...
  async getEventsFromSingleCalendar(calendarId, timeMin, timeMax) {
    console.log(`🔍 Getting events from single calendar: ${calendarId}`);
  ...
  async getBoardDetails(boardId) {
    console.log(`ℹ️ Fetching Trello board details for board ID: ${boardId}`);
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Trello API error (${response.status}): ${data.message || response.statusText}`);
      }
      return data;
    } catch (error) {
  ...
  async getTrelloCards(listId) {
    console.log(`🃏 Getting Trello cards for list ID: ${listId}`);
  ...
  async createTrelloCard(listId, cardData) {
    console.log(`➕ Creating Trello card in list ID: ${listId} with name: ${cardData.name}`);
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Trello API error: ${data.message || data.error || response.statusText}`);
      }
  ...
  async moveCardToList(cardId, newListId) {
    console.log(`➡️ Moving Trello card ID: ${cardId} to list ID: ${newListId}`);
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Trello API error: ${data.message || response.statusText}`);
      }
  ...
  async syncEventsToList(events, listId, listName) {
    console.log(`🔄 Syncing ${events.length} events to Trello list: ${listName} (ID: ${listId})`);
  ...
  async moveTomorrowToToday() {
    console.log('🌅 Moving "Tomorrow" cards to "Today" list...');
  ...
  async syncTodayAndTomorrow() {
    console.log('🚀🚀 Starting sync for Today and Tomorrow...');
    try {
      ...
      console.error('❌ Sync for Today and Tomorrow failed:', error);
      throw error;
    }
  }
  ...
  async performDailyRollover() {
    ...
    try {
      ...
      const tomorrowRange = this.getTomorrowRange();
      console.log(`🗓️ Fetching new events for Tomorrow: ${new Date(tomorrowRange.start).toDateString()}`);
