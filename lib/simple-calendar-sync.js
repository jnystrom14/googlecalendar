...
export class SimpleCalendarSync {
  ...
  async moveCardToList(cardId, newListId, updateDueDate = false) {
    ...
    // If moving from Tomorrow to Today, update due date to today
    if (updateDueDate) {
      const today = new Date();
      today.setHours(23, 59, 59, 999); // Set to end of today
      params.due = today.toISOString();
    }
  ...
  formatEventForTrello(event, targetDate = null) {
    const startTime = event.start?.dateTime || event.start?.date;
    ...
    // Use targetDate if provided (for Today/Tomorrow lists), otherwise use event start time
    let dueDate = null;
    if (targetDate) {
  ...
  async syncEventsToList(events, listId, listName, targetDate = null) {
    ...
    for (const event of events) {
      ...
      try {
        const cardData = this.formatEventForTrello(event, targetDate);
  ...
  async moveTomorrowToToday() {
    ...
    for (const card of tomorrowCards) {
      try {
        await this.moveCardToList(card.id, this.config.TODAY_LIST_ID, true);
        moved++;
  ...
  async syncTodayAndTomorrow() {
    try {
      ...
      const [todayResult, tomorrowResult] = await Promise.all([
        this.syncEventsToList(todayEvents, this.config.TODAY_LIST_ID, 'Today', todayRange.start),
        this.syncEventsToList(tomorrowEvents, this.config.TOMORROW_LIST_ID, 'Tomorrow', tomorrowRange.start)
      ]);
